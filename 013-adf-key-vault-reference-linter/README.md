# Linter de referências ao Key Vault no ADF

> Azure 013 · intermediário · segurança de credenciais

Inspeciona linked services exportados e encontra credenciais em texto claro ou referências incompletas ao Azure Key Vault.

![Arquitetura](docs/arquitetura.svg)

```bash
python3 -m src.linter # retorna 2 pelo caso inseguro intencional
...
| Item | Função |
|---|---|
| Python / JSON | Análise local sem dependências |
| Azure Data Factory | Origem dos linked services simulados |
| Azure Key Vault | Armazenamento seguro referenciado |
| AzureKeyVaultSecret | Tipo que substitui o valor secreto no JSON |

## Conceitos de Engenharia de Dados aplicados

Secret management, configuração como código, shift-left security, separação entre configuração e credencial e identidade gerenciada.

## Pré-requisitos e possíveis custos

Python 3.10+; custo local zero. Key Vault e ADF podem cobrar por operações; nenhum recurso ou segredo real é acessado.

## O que foi validado

Três testes cobrem referência válida, texto claro e store inexistente. A amostra inclui um achado intencional e dados fictícios.

## Tecnologias relacionadas ainda não utilizadas

Sem SDK Azure, RBAC, Managed Identity, rotação, private endpoint, pipeline ADF, leitura de segredo ou deploy.

## Referências oficiais

- [Armazenar credenciais no Azure Key Vault](https://learn.microsoft.com/en-us/azure/data-factory/store-credentials-in-key-vault)
- [Credenciais no Azure Data Factory](https://learn.microsoft.com/en-us/azure/data-factory/credentials)
...
cd "013-adf-key-vault-reference-linter"
```

Confirme que `pwd` termina em `013-adf-key-vault-reference-linter`. Os caminhos relativos usados pelo código dependem disso.
