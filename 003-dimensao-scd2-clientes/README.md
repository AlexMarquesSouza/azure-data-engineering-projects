# Dimensão histórica de clientes com SCD Tipo 2

> Azure 003 · iniciante/intermediário · Data Warehouse e Microsoft Fabric

Mantém todas as versões de um cliente quando nome ou cidade muda. A versão anterior é encerrada e uma nova recebe chave substituta, início de vigência e indicador ativo.

![Arquitetura SCD2](docs/arquitetura.svg)
...
| Item | Função |
|---|---|
| Python 3, CSV e biblioteca padrão | Simulação local e inspecionável |
| SCD Tipo 2 | Preservar versões históricas da dimensão |
| Chave substituta | Identificar cada versão |
| Dataflow Gen2 / Fabric Data Factory | Evolução gerenciada do fluxo |
| Fabric Warehouse ou Azure SQL | Persistência recomendada da dimensão |
| `unittest` e SVG | Teste e documentação visual |

## Conceitos aplicados
- dimensão, chave natural e chave substituta;
- detecção de mudança e vigência temporal;
- registro ativo versus versões encerradas;
- preservação do contexto histórico para relatórios.

## Pré-requisitos, custos e validação
Python 3.10+; custo local zero. Foi validado um cliente alterado, um novo, um inalterado, quatro versões finais e somente uma versão ativa por cliente. Fabric e Azure SQL podem gerar custos se implantados.

## Tecnologias relacionadas ainda não utilizadas
Não usa recursos Azure reais, banco transacional, `MERGE`, exclusão lógica, hash de atributos, concorrência ou deploy.

## Referências oficiais
- [SCD Tipo 2 com Dataflow Gen2](https://learn.microsoft.com/en-us/fabric/data-factory/slowly-changing-dimension-type-two)
- [Conceitos de dimensões lentamente mutáveis](https://learn.microsoft.com/en-us/fabric/iq/plan/powertable-concept-slowly-changing-dimensions)

## Publicação
Rascunho local; revisão manual obrigatória antes de publicação ou implantação.
...
cd "003-dimensao-scd2-clientes"
```

Confirme que `pwd` termina em `003-dimensao-scd2-clientes`. Os caminhos relativos usados pelo código dependem disso.
