# Planejador de reprocessamento para tumbling windows

> Azure 012 · intermediário · Azure Data Factory

Calcula quais triggers são afetados por uma falha e produz a ordem segura de reexecução, sem tocar em uma factory real.

![Arquitetura](docs/arquitetura.svg)

```bash
python3 -m src.planner --failed bronze_hourly
...
| Item | Função |
|---|---|
| Python / JSON | Planejamento local e configuração versionável |
| Azure Data Factory | Serviço de orquestração representado |
| Tumbling window trigger | Execuções em janelas periódicas com estado |
| Dependências | Impedem o downstream antes do sucesso upstream |

## Conceitos de Engenharia de Dados aplicados

Grafo direcionado, ordenação topológica, blast radius, reprocessamento determinístico e dependências de pipelines.

## Pré-requisitos e possíveis custos

Python 3.10+ e custo local zero. ADF cobra por atividades e orquestração quando usado na nuvem; não há autenticação ou execução remota.

## O que foi validado

Três testes cobrem ordem de rerun, dependência inexistente e ciclo. A amostra exclui corretamente um trigger não afetado.

## Tecnologias relacionadas ainda não utilizadas

Sem SDK Azure, REST API, retry policy, offsets entre janelas, Monitor, Synapse triggers ou execução de rerun.

## Referências oficiais

- [Dependências de tumbling windows](https://learn.microsoft.com/en-us/azure/data-factory/tumbling-window-trigger-dependency)
- [Criar tumbling window triggers](https://learn.microsoft.com/en-us/azure/data-factory/how-to-create-tumbling-window-trigger)
...
cd "012-adf-tumbling-window-reprocess-planner"
```

Confirme que `pwd` termina em `012-adf-tumbling-window-reprocess-planner`. Os caminhos relativos usados pelo código dependem disso.
