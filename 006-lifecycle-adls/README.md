# Simulador de lifecycle para ADLS

> Azure 006 · intermediário · armazenamento e FinOps

Avalia um inventário sem alterar dados: mantém arquivos recentes, sugere `cool` após 30 dias, `archive` após 180 dias e exclusão de temporários após 90 dias.

![Arquitetura](docs/arquitetura.svg)

## Executar e testar
```bash
python3 -m src.lifecycle
...
| Item | Função |
|---|---|
| Python 3, CSV e JSON | Simulação e relatório de impacto |
| Azure Blob Storage/ADLS Gen2 | Data lake futuro |
| Lifecycle Management | Automatizar transições e exclusões |
| Hot, Cool e Archive | Classes com custos e disponibilidade diferentes |
| Prefix filters | Aplicar regras somente a áreas específicas |
| `unittest` e SVG | Teste e documentação visual |

## Conceitos aplicados
- ciclo de vida, retenção e classificação por idade;
- otimização de custo de armazenamento;
- dry run antes de ações destrutivas;
- política diferenciada para dados temporários.

## Pré-requisitos, custos e validação
Python 3.10+, custo local zero. Foi validada uma decisão em cada categoria. Na Azure, armazenamento, leitura, reidratação e exclusão antecipada podem gerar custos.

## Tecnologias relacionadas ainda não utilizadas
Não há Storage Account, API Azure, versões, snapshots, soft delete, imutabilidade, tags ou aplicação real da política.

## Referências oficiais
- [Visão geral do lifecycle management](https://learn.microsoft.com/en-us/azure/storage/blobs/lifecycle-management-overview)
- [Estrutura das políticas](https://learn.microsoft.com/en-us/azure/storage/blobs/lifecycle-management-policy-structure)

## Publicação
Rascunho local e somente leitura; nenhum blob foi movido ou excluído.
...
cd "006-lifecycle-adls"
```

Confirme que `pwd` termina em `006-lifecycle-adls`. Os caminhos relativos usados pelo código dependem disso.
