...
> Azure 005 · intermediário · Data Factory e Azure Monitor

Consolida execuções e calcula taxa de sucesso, duração p95, falhas e violações de SLA. O exemplo gera alertas locais para uma falha e uma execução acima de cinco minutos.
...
| Item | Função |
|---|---|
| Python 3, CSV e JSON | Agregação e relatório local |
| Azure Data Factory | Origem futura do histórico de execuções |
| Azure Monitor Metrics | Indicadores operacionais |
| Diagnostic settings e Log Analytics | Retenção e consulta de logs |
| Azure Monitor Alerts | Notificação de falhas e limites |
| SLA e percentil p95 | Medir confiabilidade e duração típica superior |
| `unittest` e SVG | Validação e documentação visual |

## Conceitos aplicados
- observabilidade, métricas, logs e alertas;
- taxa de sucesso e duração p95 por pipeline;
- SLA de duração e detecção de violações;
- alertas acionáveis separados de métricas informativas.

## Pré-requisitos, custos e validação
Python 3.10+, custo local zero. Foram validadas cinco execuções, taxa de sucesso `66,67%` para ingestão, uma falha e uma violação de SLA. Log Analytics e alertas podem gerar custos quando implantados.

## Tecnologias relacionadas ainda não utilizadas
Não usa ADF real, Azure Monitor, KQL, Action Groups, retenção histórica, dashboards ou deploy.

## Referências oficiais
- [Monitorar o Azure Data Factory](https://learn.microsoft.com/en-us/azure/data-factory/monitor-data-factory)
- [Categorias de logs do Data Factory](https://learn.microsoft.com/en-us/azure/azure-monitor/reference/supported-logs/microsoft-datafactory-factories-logs)

## Publicação
Rascunho local; nenhum alerta, workspace ou repositório remoto foi criado.
...
cd "005-observabilidade-pipelines"
```

Confirme que `pwd` termina em `005-observabilidade-pipelines`. Os caminhos relativos usados pelo código dependem disso.
