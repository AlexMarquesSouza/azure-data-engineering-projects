...
> Projeto Azure 002 · iniciante · ingestão batch incremental

Extrai somente clientes alterados depois da última execução confirmada. É uma simulação local do padrão de watermark usado pelo Azure Data Factory, sem criar recursos Azure.

![Fluxo incremental](docs/arquitetura.svg)

## Executar e testar

```bash
python3 -m src.incremental
python3 -m unittest discover -s tests -v
```

Apague `state/watermark.json` para simular a primeira carga novamente.

## Ferramentas e recursos utilizados

| Item | Função |
|---|---|
| Python 3 e biblioteca padrão | Leitura, filtro e escrita do lote |
| CSV | Fonte e saída didáticas |
| JSON | Persistência local do watermark |
| Azure Data Factory | Serviço Azure ao qual o padrão pode ser levado |
| Copy Activity e Lookup | Evolução para copiar o delta e consultar os limites |
| Azure Blob Storage/ADLS | Destino recomendado para arquivos incrementais |
| `unittest` | Verificação da primeira carga e da idempotência |
| SVG | Diagrama versionável |

## Conceitos aplicados

- carga completa inicial e cargas delta posteriores;
- watermark como limite inferior exclusivo;
- estado persistente, ordenação temporal e idempotência;
- redução do volume transferido e do tempo de processamento.

## Pré-requisitos, custos e validação

A execução local requer somente Python 3.10+ e não gera custos. Foram validados o primeiro lote, a reexecução vazia e o avanço do watermark. Azure Data Factory e Storage podem gerar custos apenas numa evolução implantada.

## Tecnologias relacionadas ainda não utilizadas

Não há Data Factory real, Azure SQL, ADLS, CDC, autenticação, deploy ou tratamento de exclusões. O próximo passo natural é implementar o mesmo fluxo com Lookup, Copy Activity e atualização transacional do watermark.

## Referências oficiais

- [Carga incremental no Azure Data Factory](https://learn.microsoft.com/en-us/azure/data-factory/tutorial-incremental-copy-overview)
- [Carga incremental de múltiplas tabelas](https://learn.microsoft.com/en-us/azure/data-factory/tutorial-incremental-copy-multiple-tables-portal)

## Publicação

Rascunho local. Revisão manual obrigatória antes de criar repositório, publicar ou implantar.
...
cd "002-carga-incremental-watermark"
```

Confirme que `pwd` termina em `002-carga-incremental-watermark`. Os caminhos relativos usados pelo código dependem disso.
